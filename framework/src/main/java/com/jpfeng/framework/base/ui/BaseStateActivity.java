 ...
 * Date: 2018/4/26
 */
public abstract class BaseStateActivity<P extends IBasePresenter> extends BaseMVPActivity<P> implements IStateView {
    ...
    protected void initPage() {
        mFlRoot = findViewById(R.id.fl_activity_base_root);
        mContentView = LayoutInflater.from(getApplicationContext())
                .inflate(getContentView(), mFlRoot, false);
    ...
    protected void setCustomEmptyView(@LayoutRes int layoutID) {
        ...
        mEmptyView = LayoutInflater.from(getApplicationContext()).inflate(layoutID, mFlRoot, false);
    ...
    protected void setCustomErrorView(@LayoutRes int layoutID) {
        ...
        mErrorView = LayoutInflater.from(getApplicationContext()).inflate(layoutID, mFlRoot, false);
    ...
    protected void setCustomLoadingView(@LayoutRes int layoutID) {
        ...
        mLoadingView = LayoutInflater.from(getApplicationContext()).inflate(layoutID, mFlRoot, false);
    ...
    public void showPageEmpty() {
        if (null == mEmptyView) {
            mEmptyView = LayoutInflater.from(getApplicationContext())
    ...
    public void showPageError(String msg) {
        if (null == mErrorView) {
            mErrorView = LayoutInflater.from(getApplicationContext())
    ...
    public void showPageLoading() {
        if (null == mLoadingView) {
            mLoadingView = LayoutInflater.from(getApplicationContext())
